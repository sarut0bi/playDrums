...
module.exports.openBrowser = async (options = {}) => {
  ...
  if (options.url)
    return await this.openContext(options);
};
...
module.exports.openContext = async (options = {}) => {
  ...
  if (options.url)
    return await this.openPage(options);
};
...
module.exports.switchPage = async arg => {
  ...
  pages = Object.prototype.toString.call(arg).includes('RegExp') ?
    pages.filter(page => page.url().match(new RegExp(arg)) || page.currentTitle.match(new RegExp(arg)) || page.pageName.match(new RegExp(arg))) :
    pages.filter(page => page.url() === arg || page.currentTitle === arg || page.pageName === arg);
...
module.exports.screenshot = async (selector, options = {}) => {
  validate();
  let elementHandle = selector && selector instanceof Selector ? await selector.getElementHandle() : page;
...
module.exports.waitFor = async (elementOrFunc, args, options = {}) => {
  ...
  if (elementOrFunc instanceof Selector) {
    ...
    return await elementOrFunc.getElementHandle({ ...options, 'implicitWait': true });
  }
