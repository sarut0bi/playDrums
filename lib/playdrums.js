...
module.exports.switchPage = async arg => {
  ...
  await Promise.all(pages.map(async page => { page.title = await page.title(); return page; }));
...
module.exports.intercept = async (requestUrl, option, count) => {
  ...
  await context.route(new RegExp(requestUrl), (request) => {
    ...
    [matchRequest] = context.interceptedRequests.filter(r => request._request.url().match(r.requestUrl));
...
module.exports.clearIntercept = async (requestUrl) => {
  validate();
  requestUrl ?
    context.interceptedRequests = context.interceptedRequests.filter(r => r.requestUrl !== requestUrl) :
    context.interceptedRequests = [];
};
...
_navigationFunction = async (func, options = {}) => {
  options = { ...userOptions, ...options };
  var promises = [func, this.waitFor(500)];
...
module.exports.attach = async (filePath, to) => {
  page.once('filechooser', async ({ element, multiple }) => {
...
function Selector(selector, options) {
  ...
  this.exists = async () => {
    var element = await this.getElementHandle({ implicitWait: false });
    return element != undefined;
  }

  this.text = async () => {
    var element = await this.getElementHandle();
  ...
  this.value = async () => {
    var element = await this.getElementHandle();
    var temp = await (await element.getProperty('value')).jsonValue();
    return await (await element.getProperty('value')).jsonValue();
  }

  this.getElementHandle = async (options) => {
    var frames = await page.frames();
    if (options && options.implicitWait) {
      return await Promise.race(frames.map(frame => {
        frame.waitForSelector(selector, options);
      }));
    } else {
      for (let frame of frames) {
        let elementHandle = await frame.$(selector);
        if (elementHandle)
...
module.exports.waitFor = async (time, elementOrFunc, args, options = {}) => {
  ...
  if (typeof elementOrFunc == "function") {
    ...
    if (Array.isArray(args))
      args = await Promise.all(args.map(async arg => { if (arg instanceof Selector) return await arg.getElementHandle(); else return arg }));
    if (args && args instanceof Selector)
      args = await args.getElementHandle();
 ...
 * @param {any} args - single or multiple callback args.<br>
