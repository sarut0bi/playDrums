...
_navigationFunction = async (func, options = {}) => {
  ...
  if (options.waitForRequest)
    promises.push(...page.waitForRequest(options.waitForRequest));
  if (options.waitForResponse)
    promises.push(...page.waitForResponse(options.waitForResponse));
...
module.exports.mouseAction = async (selector, action, coordinates, options = {}) => {
  ...
  if (selector) {
    ...
    coordinates = { x: boundingBox.x + coordinates.x, y: boundingBox.y + coordinates.y };
  }
  const actions = {
    press: page.mouse.down,
    move: page.mouse.move,
    release: page.mouse.up
  }
  if (typeof action == 'string' && (action === 'press' || action === 'move' || action === 'release'))
    await actions[action](coordinates);
  else
...
function Selector(selector, options) {
  ...
  this.exists = async () => {
    const element = await this.getElementHandle({ implicitWait: false });
  ...
  this.elements = async (options = {}) => {
    ...
    let frames = await page.frames();
    let elementHandles = await Promise.all(frames.map(async frame => await frame.$$(selector)));
    return elementHandles.filter(elementHandle => elementHandle.length != 0).flat();   
  }
}
