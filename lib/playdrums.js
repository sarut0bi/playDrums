...
module.exports.openPage = async (options = {}) => {
  ...
  page.uuid = uuidv4().toString();
  browserHistory.push({ event: 'new', context: context, page: page });

  if (options.url)
    await this.goto(new URL(options.url), options);
};
...
function Selector(selector, options) {
  ...
  this.exists = async (retryInterval, retryTimeout) => {
    ...
    var promises = [this.getElementHandle(), new Promise((resolve) => setTimeout(() => resolve(undefined), retryTimeout))];
